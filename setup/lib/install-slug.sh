...
#   label=$(launchd_label)        # com.nanomika-v2-<slug>
#   unit=$(systemd_unit)          # nanomika-v2-<slug>
#   image=$(container_image_base) # nanomika-agent-v2-<slug>
#
# Slug is sha1(PROJECT_ROOT)[:8] — must match the TS helper exactly so both
# halves of setup name things consistently.

_nanomika_install_slug() {
...
launchd_label() {
  printf 'com.nanomika-v2-%s' "$(_nanomika_install_slug)"
}

systemd_unit() {
  printf 'nanomika-v2-%s' "$(_nanomika_install_slug)"
}

container_image_base() {
  printf 'nanomika-agent-v2-%s' "$(_nanomika_install_slug)"
}
