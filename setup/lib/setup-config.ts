 ...
 *   key 'fooBar' → envVar 'NANOMIKA_FOO_BAR' → flag '--foo-bar'
 *
 * Surface levels:
...
interface BaseEntry {
  ...
  key: string;
  /** Override of the auto-derived NANOMIKA_<UPPER_SNAKE> env var. */
  envVar?: string;
...
export const CONFIG: Entry[] = [
  ...
  {
    key: 'assistMode',
    envVar: 'NANOMIKA_SETUP_ASSIST_MODE',
    label: 'Assist mode',
...
export function envVarFor(e: Entry): string {
  ...
  return `NANOMIKA_${e.key.replace(/[A-Z]/g, (c) => `_${c}`).toUpperCase()}`;
}
