 ...
 *
 * Skippable with NANOMIKA_SKIP_CLAUDE_ASSIST=1 for CI/scripted runs.
 */
...
export const STEP_FILES: Record<string, string[]> = {
  bootstrap: ['setup.sh', 'setup/install-node.sh', 'nanomika.sh'],
...
function buildPrompt(ctx: AssistContext, projectRoot: string): string {
  ...
  return [
    "I'm trying to set up NanoMika on my machine and ran into an issue",
