...
#
# Typical fork setups keep the upstream nanomika repo under a remote named
...
# at nanocoai/nanomika, and prints its name.
#
# Fallback: if no existing remote matches, add `upstream` pointing at
# github.com/nanocoai/nanomika and return that — keeps forks without an
# explicit upstream configured working on the first try.
#
# Explicit override: set NANOMIKA_CHANNELS_REMOTE=<name> to skip detection.

resolve_channels_remote() {
  if [ -n "${NANOMIKA_CHANNELS_REMOTE:-}" ]; then
    printf '%s' "$NANOMIKA_CHANNELS_REMOTE"
    return 0
  fi
  ...
  while IFS=$'\t' read -r remote url; do
    case "$url" in
      *qwibitai/nanomika*|*nanocoai/nanomika*)
        printf '%s' "$remote"
        return 0
        ;;
    esac
  ...
  # remote-add failure.
  git remote add upstream https://github.com/nanocoai/nanomika.git 2>/dev/null || true
  printf '%s' "upstream"
}
