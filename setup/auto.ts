 ...
 *
 * Config via env:
 *   NANOMIKA_DISPLAY_NAME  how the agents address the operator — skips the
 *                          prompt. Defaults to $USER.
 *   NANOMIKA_AGENT_NAME    messaging-channel agent name (consumed by the
...
async function main(): Promise<void> {
  ...
  if (process.env.NANOMIKA_REEXEC_SG !== '1') {
  ...
  if (!skip.has('service')) {
    const res = await runQuietStep('service', {
      running: 'Starting NanoMika in the background…',
      done: 'NanoMika is running.',
    });
    if (!res.ok) {
      await fail('service', "Couldn't start NanoMika.", 'See logs/nanomika.error.log for details.');
    }
    if (res.terminal?.fields.DOCKER_GROUP_STALE === 'true') {
      p.log.warn(brandBody("NanoMika's permissions need a tweak before it can reach Docker."));
      p.log.message(
        brandBody(
  ...
  async function resolveDisplayName(): Promise<string> {
    ...
    const preset = process.env.NANOMIKA_DISPLAY_NAME?.trim();
    ...
    if (!skip.has('first-chat')) {
      ...
      if (ping === 'ok') {
        ...
        await offerClaudeOnFailure({
          ...
          msg:
            ping === 'socket_error'
              ? "NanoMika service isn't listening on its CLI socket."
    ...
    if (!res.ok) {
      ...
      if (service === 'running_other_checkout') {
        ...
        notes.push(
          wrapForGutter(
            [
              '• Your NanoMika service is running from a different folder on this machine.',
  ...
  const rows: [string, string][] = [
    ...
    ["See what's happening:", 'tail -f logs/nanomika.log'],
  ...
  note(
    wrapForGutter(
      "NanoMika runs on this machine. It's only reachable while this computer is on and connected to the internet. For always-on availability, run it on a cloud VM — or keep this machine awake.",
      6,
    ),
    'Heads up',
  );
...
async function confirmAssistantResponds(): Promise<PingResult> {
  ...
  if (result === 'ok') {
    ...
    const msg =
      result === 'socket_error' ? "Couldn't reach the NanoMika service." : "Your assistant didn't reply in time.";
...
function renderPingFailureNote(result: PingResult): void {
  const body =
    result === 'socket_error'
      ? [
          wrapForGutter(
            "The NanoMika service isn't listening on its local socket. Try restarting it, then chat with `pnpm run chat hi`:",
            6,
          ),
          '',
      ...
      : wrapForGutter(
          'No reply from your assistant within 30 seconds. Check `logs/nanomika.log` for clues, then try `pnpm run chat hi`.',
          6,
        );
...
function sendChatMessage(message: string): Promise<void> {
  return new Promise((resolve) => {
    // `pnpm --silent` suppresses the `> nanomika@… chat` preamble so the
...
async function runAuthStep(): Promise<void> {
  ...
  if (method === 'skip') {
    ...
    p.log.warn(
      brandBody(
        'Claude sign-in skipped. Re-run setup or run `bash nanomika.sh` to finish later.',
      ),
    );
    return;
  }
...
function maybeReexecUnderSg(): void {
  if (process.env.NANOMIKA_REEXEC_SG === '1') return;
  ...
  const existingSkip = (process.env.NANOMIKA_SKIP ?? '').split(',').map((s) => s.trim()).filter(Boolean);
  const skipList = [...new Set([...existingSkip, ...setupLog.completedStepNames()])].join(',');
  const res = spawnSync('sg', ['docker', '-c', 'pnpm run setup:auto'], {
    stdio: 'inherit',
    env: { ...process.env, NANOMIKA_REEXEC_SG: '1', ...(skipList ? { NANOMIKA_SKIP: skipList } : {}) },
  });
...
function printIntro(): void {
  const isReexec = process.env.NANOMIKA_REEXEC_SG === '1';
  const wordmark = `${k.bold('Nano')}${brandBold('Mika')}`;
...
/**
 * Bootstrap (nanomika.sh) normally initializes logs/setup.log and writes
...
function initProgressionLog(): void {
  if (process.env.NANOMIKA_BOOTSTRAPPED === '1') return;
  let commit = '';
  try {
