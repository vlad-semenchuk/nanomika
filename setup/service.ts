...
export async function run(_args: string[]): Promise<void> {
  ...
  // `com.nanomika` plist) will keep trashing this install's containers on
  ...
  if (peerReport.unloaded.length > 0) {
    log.warn('Unloaded unhealthy peer NanoMika services', {
...
function setupLaunchd(
  ...
  // Per-checkout service label so multiple NanoMika installs can coexist
...
<dict>
    ...
    <string>${projectRoot}/logs/nanomika.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanomika.error.log</string>
</dict>
</plist>`;
...
/**
 * Kill any orphaned nanomika node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    log.info('Stopped any orphaned nanomika processes');
  } catch {
...
function setupSystemd(
  ...
  const unit = `[Unit]
Description=NanoMika Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nanomika.log
StandardError=append:${projectRoot}/logs/nanomika.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nanomika processes to avoid channel connection conflicts
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanomika.sh');
  const pidFile = path.join(projectRoot, 'nanomika.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanomika.sh — Start NanoMika without systemd',
    ...
    '    echo "Stopping existing NanoMika (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoMika..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanomika.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanomika.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoMika started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanomika.log"`,
  ];
