...
#
# NanoMika needs `signal-cli` on PATH to talk to Signal. Picks the right
...
#
# Emits the standard NanoMika STATUS block on success or failure so the
...
emit_status() {
  ...
  echo "=== NANOMIKA SETUP: INSTALL_SIGNAL_CLI ==="
  echo "STATUS: ${status}"
