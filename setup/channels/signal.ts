...
export async function runSignalChannel(displayName: string): Promise<ChannelFlowResult> {
  note(
    [
      "NanoMika links to Signal as a *secondary* device on your existing",
...
async function ensureSignalCli(): Promise<void> {
  ...
  note(
    [
      "NanoMika talks to Signal through signal-cli, which isn't installed yet.",
...
async function restartService(): Promise<void> {
  const s = p.spinner();
  s.start('Restarting NanoMika so it sees your Signal account…');
  const start = Date.now();
  ...
  try {
    ...
    s.stop(`NanoMika restarted. ${k.dim(`(${fmtDuration(Date.now() - start)})`)}`);
...
async function resolveAgentName(): Promise<string> {
  const preset = process.env.NANOMIKA_AGENT_NAME?.trim();
  if (preset) {
