#!/usr/bin/env bash
#
# NanoMika — end-to-end setup entry point.
#
...
#   NANOMIKA_SKIP  comma-separated setup:auto step names to skip
...
# NanoMika splash — under-the-sea lobster mascot in truecolor braille,
# with the figlet wordmark and taglines below. Pre-rendered into
# assets/setup-splash.txt (built from assets/nanomika-icon.png via chafa +
...
# NANOMIKA_BOOTSTRAPPED=1 and skips re-printing the wordmark.
cat "$PROJECT_ROOT/assets/setup-splash.txt"

# ─── pre-flight: minimum hardware specs ────────────────────────────────
# NanoMika runs an agent container per session. Below this threshold the
...
if [ "$LOW_MEM" = true ]; then
  printf '  %s\n' "$(red 'Warning: this machine likely cannot run NanoMika.')"
  printf '  %s\n' "$(dim 'NanoMika recommends a 4 GB+ RAM machine. Below this, the host + agent')"
...
# NanoMika is known to not run reliably on Google Compute Engine instances.
...
if [ "$(uname -s)" = "Linux" ] \
  ...
  printf '  %s\n' "$(dim 'Google blocks sudo commands, so NanoMika is unlikely to run successfully on this VM.')"
  printf '  %s\n\n' "$(dim 'If you want to run NanoMika successfully, switch to a different provider (Hetzner, Hostinger, exe.dev and others..).')"
...
if [ "$(uname -s)" = "Linux" ] && [ "$(id -u)" -eq 0 ]; then
  ...
  printf '  %s\n' \
    "$(dim "Running NanoMika as root is not recommended. It can cause permission")"
  printf '  %s\n\n' \
    "$(dim "issues with containers, services, and file ownership.")"
  printf '  %s\n' "$(bold '1)') $(dim 'Show me instructions for creating a new Linux user')"
  printf '  %s\n\n' "$(bold '2)') $(dim 'Continue setting up NanoMika as root user (not recommended)')"
  ...
  case "${ROOT_ANS:-1}" in
    ...
    *)
      ...
      printf '  %s\n' "$(dim '1. Create a new user:           adduser nanomika')"
      printf '  %s\n' "$(dim '2. Add to sudo group:           usermod -aG sudo nanomika')"
      printf '  %s\n' "$(dim '3. Enable passwordless sudo:    echo "nanomika ALL=(ALL) NOPASSWD:ALL" | tee /etc/sudoers.d/nanomika')"
      printf '  %s\n' "$(dim '4. Log out:                     exit')"
      printf '  %s\n' "$(dim '5. Log back in as the new user: ssh nanomika@your-server')"
      printf '  %s\n' "$(dim '6. Clone the repo:              git clone https://github.com/nanocoai/nanomika.git && cd nanomika')"
      printf '  %s\n\n' "$(dim '7. Re-run setup:               bash nanomika.sh')"
      exit 1
      ;;
  esac
fi
...
if [ "$(uname -s)" = "Darwin" ] && ! command -v brew >/dev/null 2>&1; then
  printf '  %s\n' \
    "$(dim "Homebrew isn't installed. NanoMika uses it to install Node and Docker on your Mac.")"
  printf '  %s\n\n' \
  ...
  case "${BREW_ANS:-Y}" in
    [Yy]*|'')
      ...
      if ! command -v brew >/dev/null 2>&1; then
        ...
        printf '  %s\n\n' \
          "$(dim 'Install manually from https://brew.sh and re-run: bash nanomika.sh')"
        exit 1
      fi
      printf '\n'
      ;;
    *)
      printf '\n  %s\n\n' \
        "$(dim 'NanoMika needs Homebrew. Install it from https://brew.sh and re-run.')"
      exit 1
      ;;
  esac
fi
...
BOOTSTRAP_EXIT_FILE=$(mktemp -t nanomika-bootstrap-exit.XXXXXX)
(
  ...
  export NANOMIKA_BOOTSTRAP_LOG="$BOOTSTRAP_RAW"
...
# NANOMIKA_BOOTSTRAPPED=1 tells setup/auto.ts to skip the wordmark (we
# already printed it) and to append to the progression log rather than
# wipe it.
export NANOMIKA_BOOTSTRAPPED=1
...
# --silent suppresses pnpm's `> nanomika@2.0.0 setup:auto / > tsx setup/auto.ts`
