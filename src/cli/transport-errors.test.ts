...
describe('formatTransportError', () => {
  it('renders per-install service names on ENOENT, not the bare v1 names', () => {
    const out = formatTransportError(new Error('connect ENOENT /tmp/nanomika.sock'));

    // Regression for #2484: pre-fix, this string was a hardcoded
    // `com.nanomika` / `nanomika`, which doesn't match the actual
    ...
    expect(out).not.toMatch(/gui\/\$\(id -u\)\/com\.nanomika\b(?!-v2)/);
    expect(out).not.toMatch(/systemctl --user restart nanomika\b(?!-v2)/);
  });
