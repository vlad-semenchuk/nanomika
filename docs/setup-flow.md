...
This document is the contract for NanoMika's end-to-end scripted setup
(`bash nanomika.sh` → `pnpm run setup:auto`). Read it before adding a new
...
| File | Role |
|---|---|
| `nanomika.sh` | Top-level wrapper. Phase 1 (bootstrap) and phase 2 (setup:auto) orchestration. Writes bootstrap's raw log + progression entry. |
