...
**`nanocoai/nanomika`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`nanomika-whatsapp`, `nanomika-telegram`, `nanomika-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges nanomika-whatsapp fork  → adds WhatsApp
...
```bash
# In your local nanomika checkout
git checkout main && git pull

# For a fork:
git fetch nanomika-whatsapp
git checkout -B whatsapp-merge nanomika-whatsapp/main
git merge main
...
git push nanomika-whatsapp HEAD:main
...
1. Fork `nanomika` to `nanomika-{channel}`
