...
NANOMIKA_CHANNELS="telegram" bash migrate-v2.sh
...
1. Check both services aren't running: `systemctl --user list-units 'nanomika*'`
2. Check error log: `tail logs/nanomika.error.log`
...
1. The v2 service name is `nanomika-v2-<hash>` — find it: `systemctl --user list-units 'nanomika*'`
2. Manually stop: `systemctl --user stop <unit> && systemctl --user disable <unit>`
3. Restart v1: `systemctl --user start nanomika`
