...
│  │    • mcp__nanomika__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── nanomika.log               # Host stdout
│   └── nanomika.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanomika.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanomika-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanomika installation
...
NanoMika uses a hierarchical memory system based on CLAUDE.md files.
...
NanoMika has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nanomika__schedule_task]
        {
...
### NanoMika MCP (built-in)

The `nanomika` MCP server is created dynamically per agent call with the current group's context.
...
NanoMika runs as a single macOS launchd service.

### Startup Sequence

When NanoMika starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoMika containers from previous runs
...
### Service: com.nanomika

**launchd/com.nanomika.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nanomika.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanomika.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanomika.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.nanomika.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.nanomika.plist

# Check status
launchctl list | grep nanomika

# View logs
tail -f logs/nanomika.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanomika` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NanoMika auto-starts container runtime but may fail |
...
- `logs/nanomika.log` - stdout
- `logs/nanomika.error.log` - stderr
