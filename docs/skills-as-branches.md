...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way NanoMika is extended.

NanoMika has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.nanomika/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`nanocoai/nanomika`) maintains:

- `main` — core NanoMika (no skill code)
...
- `/setup`, `/debug`, `/update-nanomika`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`nanocoai/nanomika-skills`)
...
claude plugin install nanomika-skills@nanomika-skills --scope project
```
...
NanoMika's `.claude/settings.json` registers the official marketplace:
...
```
nanocoai/nanomika-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    nanomika-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `nanomika-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-nanomika` — after merging main, optionally check for skill updates
...
1. Fork `nanocoai/nanomika` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/nanomika.git
   cd nanomika
   ```
3. Run Claude Code:
   ```bash
...
Users who previously ran `git clone https://github.com/nanocoai/nanomika.git` and have local customizations:

1. Fork `nanocoai/nanomika` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/nanomika.git
...
3. From this point, `origin` = their fork, `upstream` = nanocoai/nanomika
...
In both cases:
- Delete the `.nanomika/` directory (no longer needed)
...
This is the same as the existing `/update-nanomika` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-nanomika` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to nanocoai/nanomika:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`nanocoai/nanomika-skills`)
...
1. Maintains a fork of NanoMika (e.g., `alice/nanomika`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/nanomika-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to NanoMika's `.claude/settings.json`:
...
```json
{
  "extraKnownMarketplaces": {
    ...
    "alice-nanomika-skills": {
      "source": {
        "source": "github",
        "repo": "alice/nanomika-skills"
      }
    }
  }
}
...
Once merged, all NanoMika users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-nanomika-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/nanomika.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/nanomika-skills` to discover skills from any source.
...
A flavor is a curated fork of NanoMika — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "NanoMika for Sales," "NanoMika Minimal," "NanoMika for Developers").
...
AskUserQuestion: "Start with a flavor or default NanoMika?"
- Default NanoMika
- NanoMika for Sales — Gmail + Slack + CRM (maintained by alice)
- NanoMika Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/nanomika.git
...
- `upstream` — `nanocoai/nanomika` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `nanomika-flavor` for searchability
...
- `.nanomika/` state directory

Operational skills (`setup`, `debug`, `update-nanomika`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/nanocoai/NanoMika.git
cd NanoMika
claude
```
...
```
1. Fork nanocoai/nanomika on GitHub
2. git clone https://github.com/<you>/nanomika.git
3. cd nanomika
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/nanocoai/nanomika.git`
- Check if `origin` points to the user's fork (not nanocoai). If it points to nanocoai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install nanomika-skills@nanomika-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-nanomika, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`nanocoai/nanomika-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in NanoMika. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.nanomika/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.nanomika/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to NanoMika's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
