...
class StatusBarController: NSObject {
    ...
    private let plistPath = "\(NSHomeDirectory())/Library/LaunchAgents/com.nanomika.plist"

    /// Derive the NanoMika project root from the binary location.
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let image = NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "NanoMika") {
                image.isTemplate = true
                button.image = image
            } else {
                button.title = "⚡"
            }
            button.toolTip = "NanoMika"
        }
    }
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.nanomika"]
        let pipe = Pipe()
    ...
    private func updateMenu() {
        ...
        let label = isRunning ? "NanoMika is running" : "NanoMika is stopped"
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.nanomika"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/nanomika.log"
