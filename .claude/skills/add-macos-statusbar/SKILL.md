...
description: Add a macOS menu bar status indicator for NanoMika. Shows a bolt icon with a green/red dot indicating whether NanoMika is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows NanoMika's running status and lets the user
...
```bash
launchctl list | grep com.nanomika.statusbar
```
...
Create `~/Library/LaunchAgents/com.nanomika.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.nanomika.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.nanomika.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see NanoMika's status and control the service.
>
> - **Green dot** — NanoMika is running
> - **Red dot** — NanoMika is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.nanomika.statusbar.plist
rm ~/Library/LaunchAgents/com.nanomika.statusbar.plist
rm dist/statusbar
```
