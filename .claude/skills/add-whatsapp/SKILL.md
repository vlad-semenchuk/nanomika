...
NanoMika doesn't ship channels in trunk. This skill copies the native WhatsApp (Baileys) adapter and its `whatsapp-auth` setup step in from the `channels` branch. No Chat SDK bridge.
...
2. Connected: `grep "Connected to WhatsApp" logs/nanomika.log | tail -1`
...
Two instances connected with same credentials. Ensure only one NanoMika process is running.
