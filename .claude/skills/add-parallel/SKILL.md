...
Adds Parallel AI MCP integration to NanoMika for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NanoMika scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoMika already set up and running
...
Add Parallel AI MCP servers after the nanomika server:
```typescript
const mcpServers: Record<string, any> = {
  nanomika: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nanomika__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nanomika__send_message
   4. Use mcp__nanomika__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanomika-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/nanomika.log | grep "scheduled task"`
