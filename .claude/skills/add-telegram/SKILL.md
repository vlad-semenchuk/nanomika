...
NanoMika doesn't ship channels in trunk. This skill copies the Telegram adapter, its formatting/pairing helpers, their tests, and the `pair-telegram` setup step in from the `channels` branch.
...
2. Send `/newbot` and follow the prompts:
   - Bot name: Something friendly (e.g., "NanoMika Assistant")
   - Bot username: Must end with "bot" (e.g., "nanomika_bot")
