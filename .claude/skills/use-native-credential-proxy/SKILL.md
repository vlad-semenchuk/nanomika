...
This skill replaces the OneCLI gateway with NanoMika's built-in credential proxy. Containers get credentials injected via a local HTTP proxy that reads from `.env` — no external services needed.
...
If neither check matches, you may be on an older version. Run `/update-nanomika` first, then retry.
...
```bash
tail -20 logs/nanomika.log | grep "Credential proxy"
```
