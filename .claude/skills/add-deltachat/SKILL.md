...
| `DC_DISPLAY_NAME` | `NanoMika` | Bot display name shown in DeltaChat |
...
```bash
grep "invite link" logs/nanomika.log | tail -1
...
```bash
grep "Channel credentials missing" logs/nanomika.log | grep deltachat
```
...
```bash
grep "DeltaChat" logs/nanomika.log | tail -20
```
...
1. Check the service is running and the adapter started: `grep "Channel adapter started.*deltachat" logs/nanomika.log`
2. Check connectivity: `grep "DeltaChat: IO started" logs/nanomika.log`
...
```bash
grep "DeltaChat" logs/nanomika.error.log | tail -20
```
