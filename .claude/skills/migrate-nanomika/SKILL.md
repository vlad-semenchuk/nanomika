...
NanoMika users fork the repo and customize it — changing config values, editing source files, modifying personas, adding skills. When upstream ships updates or refactors, `git merge` produces painful conflicts because the same core files were changed on both sides.
...
Check remotes with `git remote -v`. If `upstream` is missing, ask for the URL (default: `https://github.com/nanocoai/nanomika.git`), add it, then `git fetch upstream --prune`.
...
Check for existing guide: `.nanomika-migrations/guide.md` or `.nanomika-migrations/index.md`.

**Determine the tier based on the total diff from base:**

### Tier 1: Lightweight — suggest `/update-nanomika` instead
...
Tell the user the scope is small and suggest `/update-nanomika` might be simpler. Let them choose.
...
For Tier 1:
- **Use /update-nanomika** — simpler merge-based approach
...
> Explore this NanoMika fork to identify all changes from the upstream base. Run these commands and report back:
>
...
**Storage:** `.nanomika-migrations/guide.md` for Tier 2. `.nanomika-migrations/` directory with `index.md` and section files for Tier 3.
...
1. Stop the service (do this directly):
   ```bash
   launchctl unload ~/Library/LaunchAgents/com.nanomika.plist 2>/dev/null || true
   ```
...
cp -r "$WORKTREE_PATH/.nanomika-migrations" /tmp/nanomika-migrations-backup 2>/dev/null || true
...
cp -r /tmp/nanomika-migrations-backup/* .nanomika-migrations/ 2>/dev/null || true
rm -rf /tmp/nanomika-migrations-backup
```
...
1. Use the Read tool to read `.claude/skills/migrate-nanomika/diagnostics.md`.
