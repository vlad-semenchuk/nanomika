...
Add an `atomic_chat` entry alongside `nanomika`:
...
Edit `container/agent-runner/src/providers/claude.ts`. Find `'mcp__nanomika__*',` in the `TOOL_ALLOWLIST` array and add `'mcp__atomic_chat__*',` on the following line:
...
```bash
tail -f logs/nanomika.log | grep -i atomic
```
