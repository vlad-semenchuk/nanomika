---
name: x-integration
description: X (Twitter) integration for NanoMika. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NanoMika v1.0.0. Directory structure may change in future versions.
...
1. **NanoMika is installed and running** - WhatsApp connected, service active
...
| `NANOMIKA_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanomika.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoMika, make the following modifications:
...
All paths below are relative to project root (`NANOMIKA_ROOT`).
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanomika.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanomika.log | tail -20
```
...
docker run nanomika-agent ls -la /app/src/skills/
```
