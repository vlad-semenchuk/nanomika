---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to NanoMika from within Emacs (Doom, Spacemacs, or vanilla). Local HTTP bridge — no bot token or external service needed.
---
...
- **Code review** — select a region and send it with `nanomika-org-send`; the response appears as a child heading inline in your org file
...
NanoMika doesn't ship channels in trunk. This skill copies the Emacs adapter and the Lisp client in from the `channels` branch. Native HTTP bridge — no Chat SDK, no adapter package.
...
git show origin/channels:emacs/nanomika.el          > emacs/nanomika.el
```
...
`nanomika.el` needs only Emacs 27.1+ builtins (`url`, `json`, `org`) — no package manager.
...
```elisp
;; NanoMika — personal AI assistant channel
(load (expand-file-name "~/src/nanomika/emacs/nanomika.el"))

(map! :leader
      :prefix ("N" . "NanoMika")
      :desc "Chat buffer"  "c" #'nanomika-chat
      :desc "Send org"     "o" #'nanomika-org-send)
```
...
```elisp
;; NanoMika — personal AI assistant channel
(load-file "~/src/nanomika/emacs/nanomika.el")

(spacemacs/set-leader-keys "aNc" #'nanomika-chat)
(spacemacs/set-leader-keys "aNo" #'nanomika-org-send)
```
...
(global-set-key (kbd "C-c n c") #'nanomika-chat)
(global-set-key (kbd "C-c n o") #'nanomika-org-send)
```

Reload: `M-x eval-buffer` or restart Emacs.

Replace `~/src/nanomika/emacs/nanomika.el` with your actual NanoMika checkout path.

If `EMACS_AUTH_TOKEN` is set, also add (any distribution):

```elisp
(setq nanomika-auth-token "<your-token>")
```

If you changed `EMACS_CHANNEL_PORT` from the default:

```elisp
(setq nanomika-port <your-port>)
```
...
`tail -f logs/nanomika.log` should show `Emacs channel listening` at startup.
...
- Interactive chat buffer (`nanomika-chat`) with markdown → org-mode rendering
- Org integration (`nanomika-org-send`) — sends the current subtree or region; reply lands as a child heading
...
Either a stale NanoMika is running or another app has the port. Kill stale process or change port:

```bash
lsof -ti :8766 | xargs kill -9
# or set EMACS_CHANNEL_PORT in .env and mirror in Emacs config (nanomika-port)
```

### Adapter not starting

If `grep "Emacs channel listening" logs/nanomika.log` returns nothing, check that `EMACS_ENABLED=true` is in `.env` and that the adapter import is present:
...
1. NanoMika running: `launchctl list | grep "$(. setup/lib/install-slug.sh && launchd_label)"` (macOS) / `systemctl --user status "$(. setup/lib/install-slug.sh && systemd_unit)"` (Linux)
...
3. Logs show inbound: `grep 'channel_type=emacs\|Emacs' logs/nanomika.log | tail -20`
...
```elisp
M-x describe-variable RET nanomika-auth-token RET
```

Must match `EMACS_AUTH_TOKEN` in `.env`. If you didn't set one server-side, clear it in Emacs too:

```elisp
(setq nanomika-auth-token nil)
```

### nanomika.el not loading

```bash
ls ~/src/nanomika/emacs/nanomika.el
```

If NanoMika is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
```bash
rm src/channels/emacs.ts src/channels/emacs.test.ts emacs/nanomika.el
...
# Remove the NanoMika block from your Emacs config
