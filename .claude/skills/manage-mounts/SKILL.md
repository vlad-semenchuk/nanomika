...
Configure which host directories NanoMika agent containers can access. The mount allowlist lives at `~/.config/nanomika/mount-allowlist.json`.

## Show Current Config

```bash
cat ~/.config/nanomika/mount-allowlist.json 2>/dev/null || echo "No mount allowlist configured"
```
