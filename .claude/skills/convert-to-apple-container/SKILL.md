...
This skill switches NanoMika's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
git remote add upstream https://github.com/nanocoai/nanomika.git
```
...
```bash
grep -q 'nanomika proxy' /etc/pf.conf 2>/dev/null || echo '# nanomika proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanomika-agent:latest "Container OK"
```
