...
Unlike Telegram or Discord, Signal has no bot API. NanoMika registers as a full Signal account on a dedicated phone number (recommended) or links as a secondary device on your existing number.
...
Use a dedicated SIM or VoIP number. NanoMika owns it entirely.
...
> ⚠ Stop NanoMika before running signal-cli commands — the daemon holds an exclusive lock on its data directory while running.
...
Joins an existing Signal account as a secondary device. Simpler, but NanoMika shares your personal number.

```bash
signal-cli -a +1YOURNUMBER link --name "NanoMika"
```
...
# Whether NanoMika manages the daemon lifecycle (default: true).
...
```bash
grep "Signal" logs/nanomika.log | tail
```
...
1. Channel initialized: `grep "Signal channel connected" logs/nanomika.log | tail -1`
...
4. **Check for duplicate service instances** — if `logs/nanomika.error.log` shows `No adapter for channel type channelType="signal"` despite the adapter starting, two NanoMika processes are racing. See the `/debug` skill section "No adapter for channel type / Messages silently lost" for the full fix.
...
signal-cli holds an exclusive lock on its data directory while the daemon is running. Stop NanoMika before running any `signal-cli` commands directly, then restart afterward.
