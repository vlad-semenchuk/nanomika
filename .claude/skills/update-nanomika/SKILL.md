---
name: update-nanomika
description: Efficiently bring upstream NanoMika updates into a customized install, with preview, selective cherry-pick, and low token usage.
---

# About

Your NanoMika fork drifts from upstream as you customize it. This skill pulls upstream changes into your install without losing your modifications.

Run `/update-nanomika` in Claude Code.

## How it works

**Preflight**: checks for clean working tree (`git status --porcelain`). If `upstream` remote is missing, asks you for the URL (defaults to `https://github.com/nanocoai/nanomika.git`) and adds it. Detects the upstream branch name (`main` or `master`).
...
# Goal
Help a user with a customized NanoMika install safely incorporate upstream changes without a fresh reinstall and without blowing tokens.
...
- Ask the user for the upstream repo URL (default: `https://github.com/nanocoai/nanomika.git`).
...
**Large drift check:** If the upstream commit count and age suggest the user has a lot of catching up to do, mention that `/migrate-nanomika` might be a better fit — it extracts customizations and reapplies them on clean upstream instead of merging. Offer it as an option but don't push.
...
- Restart the service to apply changes. The unit/label names are per-install — derive them with `setup/lib/install-slug.sh`. Run from your NanoMika project root:
...
1. Use the Read tool to read `.claude/skills/update-nanomika/diagnostics.md`.
