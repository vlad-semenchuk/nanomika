...
- Node.js >= 20 (already required by NanoMika)

## Install

NanoMika doesn't ship channels in trunk. This skill copies the WeChat adapter in from the `channels` branch.
...
```bash
tail -f logs/nanomika.log | grep WeChat
# or
...
- **Only one instance can use a given token at a time.** Don't run multiple NanoMika instances pointing to the same `data/wechat/auth.json`.
...
- **post-install-wiring**: Use the `wire-dm.ts` helper (see the "Wire your first DM" section above) if running this skill standalone. If running as part of `bash nanomika.sh`, `init-first-agent.ts` handles wiring — just pass the `platform-id` and `admin-user-id` captured above.
