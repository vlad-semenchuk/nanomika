---
name: add-dashboard
description: Add a monitoring dashboard to NanoMika. Installs @nanoco/nanomika-dashboard and a pusher that sends periodic JSON snapshots.
---

# /add-dashboard — NanoMika Dashboard
...
```
NanoMika (pusher)              Dashboard (npm package)
┌──────────┐    POST JSON      ┌──────────────┐
│ collects │ ────────────────→ │ /api/ingest  │
...
```bash
pnpm install @nanoco/nanomika-dashboard
```
...
Add after step 7 (OneCLI approval handler), before the `log.info('NanoMika running')` line:
...
```typescript
  ...
  if (dashboardSecret) {
    const { startDashboard } = await import('@nanoco/nanomika-dashboard');
...
- **No logs**: Check `logs/nanomika.log` exists

## Removal

```bash
pnpm uninstall @nanoco/nanomika-dashboard
