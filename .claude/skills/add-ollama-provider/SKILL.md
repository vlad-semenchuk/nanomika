...
In `src/container-runner.ts`, after the `NANOMIKA_MCP_SERVERS` block, add:
...
CTR=$(docker ps --filter "name=nanomika-v2-<FOLDER>" --format "{{.Names}}" | head -1)
