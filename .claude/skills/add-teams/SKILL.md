...
Connect NanoMika to Microsoft Teams for interactive chat in team channels, group chats, and direct messages.

## Install

NanoMika doesn't ship channels in trunk. This skill copies the Teams adapter in from the `channels` branch.
...
3. Create the Entra app, client secret, and bot registration:
   ...
   The CLI prints the credentials as `CLIENT_ID`, `CLIENT_SECRET`, and `TENANT_ID`. Map them to NanoMika's env keys:
...
2. Click **New client secret**, description "nanomika", expiry 180 days
...
2. Fill in:
   - **Bot handle**: unique name (e.g., "nanomika-bot")
...
```bash
az group create --name nanomika-rg --location eastus
az bot create \
  --resource-group nanomika-rg \
  --name nanomika-bot \
...
```bash
az bot msteams create --resource-group nanomika-rg --name nanomika-bot
```
...
```json
{
  ...
  "name": { "short": "NanoMika", "full": "NanoMika Assistant" },
  "description": {
    "short": "NanoMika assistant bot",
    "full": "NanoMika personal assistant powered by Claude."
  },
...
- **how-to-find-id**: Send a message to the bot in the channel. NanoMika auto-creates a messaging group and logs the platform ID. Use that messaging group ID for wiring.
