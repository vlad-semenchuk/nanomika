...
// Deferred SDK builtins that either sidestep nanomika's own scheduling or
// don't fit our async message-passing model (they're designed for Claude
// Code's interactive UI and would hang here).
//
// - CronCreate / CronDelete / CronList / ScheduleWakeup: we have durable
//   scheduling via mcp__nanomika__schedule_task.
// - AskUserQuestion: SDK returns a placeholder instead of blocking on a
//   real answer — we have mcp__nanomika__ask_user_question that persists
...
// Tool allowlist for NanoMika agent containers. MCP-tool entries are derived
...
const preToolUseHook: HookCallback = async (input) => {
  ...
  if (SDK_DISALLOWED_TOOLS.includes(toolName)) {
    return {
      decision: 'block',
      stopReason: `Tool '${toolName}' is not available in this environment — use the nanomika equivalent.`,
...
function archiveTranscriptFile(transcriptPath: string | undefined, sessionId: string | undefined, assistantName?: string): boolean {
  ...
  try {
    ...
    const conversationsDir = process.env.NANOMIKA_CONVERSATIONS_DIR || '/workspace/agent/conversations';
