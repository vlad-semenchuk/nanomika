...
2. Once the bump commit lands on `main`, open a draft GitHub Release:
   ...
   - **Body:** copy the CHANGELOG entry verbatim. Append a `## Contributors` section listing every PR author who landed work in the release window. Append a `**Full Changelog**: https://github.com/nanocoai/nanomika/compare/<prev-tag>...vX.Y.Z` line at the bottom.
3. If anyone in the window opened their first NanoMika PR, add a `## New Contributors` section above `## Contributors`, with each first-timer's first PR link and an invite to Discord.
...
NanoMika currently ships a single channel: every published release is a stable release.
...
- **Stable** — currently identical to latest. NanoMika has no separate stable branch and no pre-release/RC channel.
