...
> Private hard fork of an upstream personal-assistant agent (upstream v2.0.70),
> renamed and de-branded for self-hosted use. Not synced with upstream — don't
> attempt to merge upstream changes into this tree.
...
- **Host-side** (nanomika): receives pending approvals and routes them to a human. `src/modules/approvals/onecli-approvals.ts` registers a callback via `onecli.configureManualApproval(cb)` (long-polls `GET /api/approvals/pending`). The callback uses `pickApprover` + `pickApprovalDelivery` from `src/modules/approvals/primitive.ts` to DM an approver. Approvers are resolved from the `user_roles` table — preference order: scoped admins for the agent group → global admins → owners. There is no env var like `NANOMIKA_ADMIN_USER_IDS`; roles are persisted in the central DB only.
...
- **Utility skills** — ship code files alongside `SKILL.md` (e.g. `/mikacli`).
- **Operational skills** — instruction-only workflows (`/setup`, `/debug`, `/customize`, `/init-first-agent`, `/manage-channels`, `/init-onecli`, `/update-nanomika`).
...
| `/update-nanomika` | Bring upstream updates into a customized install |
...
./container/build.sh  # Rebuild agent container image (nanomika-agent:latest)
...
launchctl kickstart -k gui/$(id -u)/com.nanomika  # restart

# Linux (systemd)
systemctl --user start|stop|restart nanomika
```
...
| What | Where |
|------|-------|
| Host logs | `logs/nanomika.error.log` first (delivery failures, crash-loop backoff, warnings), then `logs/nanomika.log` for the full routing chain |
...
./container/build.sh
launchctl kickstart -k gui/$(id -u)/com.nanomika   # macOS
# systemctl --user restart nanomika                # Linux
```
