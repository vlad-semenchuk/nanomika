#!/usr/bin/env bash
#
# migrate-v2.sh — Migrate a NanoMika v1 install into this v2 checkout.
#
...
#
# Finds v1 automatically (sibling directory, or $NANOMIKA_V1_PATH).
...
# the same lookup nanomika.sh does.
...
find_v1() {
  # Explicit override
  if [ -n "${NANOMIKA_V1_PATH:-}" ]; then
    if [ -f "$NANOMIKA_V1_PATH/store/messages.db" ]; then
      echo "$NANOMIKA_V1_PATH"
      return 0
    fi
    step_fail "NANOMIKA_V1_PATH=$NANOMIKA_V1_PATH does not contain store/messages.db"
    return 1
  fi

  # Scan sibling directories for any v1 install carrying a v1 DB
  local parent
...
if V1_PATH=$(find_v1); then
  ...
  echo
  echo "  $(dim 'Set NANOMIKA_V1_PATH to point at your v1 checkout:')"
  echo "  $(dim 'NANOMIKA_V1_PATH=~/nanomika bash migrate-v2.sh')"
  echo
  abort "v1-not-found"
fi
...
export NANOMIKA_V1_PATH="$V1_PATH"
export NANOMIKA_V2_PATH="$PROJECT_ROOT"
...
# Channel selection — clack multiselect (interactive) or NANOMIKA_CHANNELS env var.
# NANOMIKA_CHANNELS accepts comma-separated channel names: "telegram,discord"
SELECTED_CHANNELS=()
...
# on disk so the user can rollback with: systemctl --user start nanomika
