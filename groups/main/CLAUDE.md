...
You also have `mcp__nanomika__send_message` which sends a message immediately while you're still working. This is useful when you want to acknowledge a request before starting longer work.
...
If the user wants to set up an allowlist, edit `~/.config/nanomika/sender-allowlist.json` on the host:
...
- The config file is on the host at `~/.config/nanomika/sender-allowlist.json`, not inside the container
