...
# setup.sh — Bootstrap script for NanoMika
...
# Where verbose bootstrap logs go. nanomika.sh captures setup.sh's stdout to
...
# Default: write to the raw bootstrap log if nanomika.sh pointed us there,
# else fall back to a dedicated bootstrap log (keeps standalone `bash
# setup.sh` invocations working).
LOG_FILE="${NANOMIKA_BOOTSTRAP_LOG:-${PROJECT_ROOT}/logs/bootstrap.log}"
